a#include "diffdrive_arduino/arduino_comms.h"
...
void ArduinoComms::readEncoderValues(int &val_1, int &val_2)
{
    ...
    auto logger = rclcpp::get_logger("my_logger_encode");
    RCLCPP_INFO_STREAM(logger,"Enoder vaues received: " <<val_1<<" "<<val_2);
}
...
std::string ArduinoComms::sendMsg(const std::string &msg_to_send, bool print_output)
{
    ...
    if (print_output)
    {
        ...
        RCLCPP_INFO_STREAM(logger,"Sent: " << msg_to_send);
        RCLCPP_INFO_STREAM(logger,"Received: " << response);
    }
